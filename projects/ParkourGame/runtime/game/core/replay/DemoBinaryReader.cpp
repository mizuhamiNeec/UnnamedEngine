...
namespace Unnamed {
	bool DemoBinaryReader::ReadFile(
		const Path& path, DemoFileV2& outFile, std::string* outError
	) {
	...
	void DemoBinaryReader::SetError(
	...
	bool DemoBinaryReader::ReadHeadChunk(
		BinaryReader& reader, DemoFileV2& outFile
	) {
		std::string mapPathText;
		if (!reader.ReadPod(outFile.tickRate) ||
		    !reader.ReadPod(outFile.startTick) ||
		    !reader.ReadString(mapPathText)) {
			return false;
		}
		outFile.mapPath = Path(mapPathText);
	...
	bool DemoBinaryReader::ReadInitChunk(
		BinaryReader& reader, DemoFileV2& outFile
	) {
		uint32_t count = 0;
	...
	bool DemoBinaryReader::ReadCommandsChunk(
		BinaryReader& reader, DemoFileV2& outFile
	) {
		uint32_t count = 0;
		if (!reader.ReadPod(count)) {
	...
	bool DemoBinaryReader::ReadSnapChunk(
		BinaryReader& reader, DemoFileV2& outFile
	) {
	...
	void DemoBinaryReader::SortLoadedData(DemoFileV2& file) {
		std::ranges::sort(
