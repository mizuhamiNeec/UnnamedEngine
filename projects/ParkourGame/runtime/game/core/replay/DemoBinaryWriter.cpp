...
namespace Unnamed {
	bool DemoBinaryWriter::WriteFile(
		const Path& path, const DemoFileV2& file, std::string* outError
	) const {
	...
	void DemoBinaryWriter::SetError(
	...
	bool DemoBinaryWriter::WriteChunk(
	...
	uint32_t DemoBinaryWriter::EntityTypeId(const std::string_view text) {
	...
	bool DemoBinaryWriter::WriteHeadChunk(
		BinaryWriter& writer, const DemoFileV2& file, std::string* outError
	) {
		return WriteChunk(
			writer,
			kDemoChunkHead,
			[&]() {
				if (!writer.WritePod(file.tickRate) ||
				    ...
				    !writer.WriteString(file.mapPath.ToUtf8())) {
					return false;
				}
	...
	bool DemoBinaryWriter::WriteInitChunk(
		BinaryWriter& writer, const DemoFileV2& file, std::string* outError
	) {
		return WriteChunk(
			writer,
			kDemoChunkInit,
			[&]() {
				std::vector<EntitySnapshotRecord> initial = file.
					initialEntities;
				std::ranges::sort(
					initial,
					[](
	...
	bool DemoBinaryWriter::WriteCommandsChunk(
		BinaryWriter& writer, const DemoFileV2& file, std::string* outError
	) {
		return WriteChunk(
			writer,
			kDemoChunkCmds,
			[&]() {
				const uint32_t count = static_cast<uint32_t>(file.commands.
					size());
				if (!writer.WritePod(count)) {
					return false;
				}
				...
				for (const DemoTickCommand& command : file.commands) {
					if (command.commandType !=
					    DEMO_COMMAND_TYPE::PLAYER_INPUT) {
						return false;
					}
					...
					packed.commandType = static_cast<uint8_t>(command.
						commandType);
	...
	bool DemoBinaryWriter::WriteSnapChunk(
		BinaryWriter& writer, const DemoFileV2& file, std::string* outError
	) {
		return WriteChunk(
			writer,
			...
			[&]() {
				...
				for (const FrameSnapshot& snapshot : file.snapshots) {
					...
					for (const EntitySnapshotRecord& record : snapshot.
					     entities) {
						const uint32_t entityTypeId = record.entityTypeId !=
								0u ?
								record.entityTypeId :
