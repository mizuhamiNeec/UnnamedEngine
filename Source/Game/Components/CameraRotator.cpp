...
void CameraRotator::Update([[maybe_unused]] float deltaTime) {
	...
	// 感度と回転値を計算
	const float sensitivity = ConVarManager::GetConVar("sensitivity")->GetValueAsFloat();
	const float m_pitch = ConVarManager::GetConVar("m_pitch")->GetValueAsFloat();
	const float m_yaw = ConVarManager::GetConVar("m_yaw")->GetValueAsFloat();
	const float cl_pitchdown = ConVarManager::GetConVar("cl_pitchdown")->GetValueAsFloat();
	const float cl_pitchup = ConVarManager::GetConVar("cl_pitchup")->GetValueAsFloat();
	...
	Quaternion yawRotation = Quaternion::AxisAngle(Vec3::up, yaw_ * Math::deg2Rad);
	Quaternion pitchRotation = Quaternion::AxisAngle(Vec3::right, pitch_ * Math::deg2Rad);
