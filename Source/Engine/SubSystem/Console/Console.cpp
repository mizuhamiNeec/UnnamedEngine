...
void Console::Update() {
	...
	bool bWindowOpen = ImGui::Begin(
		(StrUtils::ConvertToUtf8(kIconTerminal) + " コンソール").c_str(),
	...
	if (bWindowOpen) {
		...
		if (ImGuiManager::IconButton(
			StrUtils::ConvertToUtf8(kIconTerminal).c_str(), "ConVar",
			{48.0f, 48.0f})) {
...
void Console::NeoFetch([[maybe_unused]] const std::vector<std::string>& args) {
	...
	const std::vector<std::string> info = {
		...
		uptime + "\n",
		"Resolution:  " +
		std::to_string(WindowManager::GetMainWindow()->GetClientWidth()) + "x" +
		std::to_string(WindowManager::GetMainWindow()->GetClientHeight()) +
		"\n",
...
void Console::ShowMenuBar() {
	...
	if (ImGui::BeginMenuBar()) {
		...
		if (ImGui::BeginMenu("Help")) {
			...
			if (ImGui::MenuItem(
				(StrUtils::ConvertToUtf8(0xeb8e) + " About Console").c_str())) {
				bShowAbout_ = true;
			}
			ImGui::EndMenu();
		}
...
void Console::ShowContextMenu() {
	...
	if (ImGui::BeginPopupContextWindow()) {
		...
		if (ImGui::MenuItem(
			(StrUtils::ConvertToUtf8(kIconCopy) + " Copy Selected").c_str())) {
			std::string copiedText;
			for (size_t i = 0; i < displayState_.buffer.size(); ++i) {
				if (displayState_.selected[i]) {
					copiedText += displayState_.buffer[i].text;
		...
		if (ImGui::MenuItem(
			(StrUtils::ConvertToUtf8(kIconVisibility) + " Show All Channels").
			c_str())) {
			currentFilterChannel_ = Channel::None;
		}
...
void Console::ShowAbout() {
	...
	if (bOpen) {
		ImGui::Text(
			(StrUtils::ConvertToUtf8(kIconTerminal) + " Unnamed Console").
			c_str());
		ImGui::Spacing();
		ImGui::Spacing();
		...
		ImGui::Spacing();
		ImGui::Spacing();
		ImGui::Text("Build: %s %s", kEngineBuildDate.c_str(),
		            kEngineBuildTime.c_str());
		ImGui::Spacing();
		ImGui::Spacing();
	...
	if (ImGui::Begin("ConVar Helper", &bShowConVarHelper_,
		...
		// メニューバー
		{
			if (ImGui::BeginMenuBar()) {
				if (ImGui::BeginMenu("File")) {
					if (ImGui::MenuItem(
						(StrUtils::ConvertToUtf8(kIconDownload) +
							" Import Page").c_str())) {
						ImportPage();
					}
					ImGui::BeginDisabled();
					if (ImGui::MenuItem(
						(StrUtils::ConvertToUtf8(kIconUpload) + " Export Page").
						c_str())) {
						ExportPage();
					}
					ImGui::EndDisabled();
					...
					for (uint32_t col = 0; col < grid.width; ++col) {
						...
						if (element.type == GridElement::Type::None) {
							...
							ImGui::PushStyleColor(
								ImGuiCol_Button, ToImVec4(element.bgColor));
							ImGui::PushStyleColor(
								ImGuiCol_ButtonActive,
								ToImVec4(element.bgColor));
							ImGui::PushStyleColor(
								ImGuiCol_ButtonHovered,
								ToImVec4(element.bgColor));
							ImGui::PushStyleColor(
								ImGuiCol_Text, ToImVec4(element.fgColor));
	...
	if (GetOpenFileName(&ofn) == TRUE) {
		...
		if (file.is_open()) {
			...
			auto      iniData = parser.ParseIniFile(
				StrUtils::ToString(ofn.lpstrFile));
...
void Console::ExportPage() {
	...
	if (GetSaveFileName(&ofn) == TRUE) {
		...
		if (file.is_open()) {
			...
			file.close();
			Print("ページを正常にエクスポートしました: " + StrUtils::ToString(ofn.lpstrFile) +
			      "\n", kConFgColorDark, Channel::Console);
		} else {
