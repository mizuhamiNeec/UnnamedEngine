...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // start a new Dear ImGui frame, you can submit any command from this point until PreRender()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by PreRender(). If you don't need to render data (skipping rendering) you may call EndFrame() without PreRender()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the draw data. You can then get call GetDrawData().
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after PreRender() and until the next call to NewFrame(). this is what you have to render.
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is updated during the frame. valid before PreRender(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
struct ImGuiIO
{
    ...
    //------------------------------------------------------------------
    // Output - Updated by NewFrame() or EndFrame()/PreRender()
    ...
    int         MetricsRenderVertices;              // Vertices output during last call to PreRender()
    int         MetricsRenderIndices;               // Indices output during last call to PreRender() = number of triangles * 3
...
// PreRender state is not reset by default because they are many perfectly useful way of altering render state (e.g. changing shader/blending settings before an Image call).
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(const ImVec2& clip_rect_min, const ImVec2& clip_rect_max, bool intersect_with_current_clip_rect = false);  // PreRender-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImDrawData
{
    bool                Valid;              // Only valid after PreRender() is called and before the next NewFrame() is called.
...
struct ImGuiViewport
{
    ...
    ImDrawData*         DrawData;               // The ImDrawData corresponding to this viewport. Valid after PreRender() and until the next call to NewFrame().
...
// - Application:  In your main loop, call ImGui::UpdatePlatformWindows(), ImGui::RenderPlatformWindowsDefault() after EndFrame() or PreRender().
...
struct ImGuiPlatformIO
{
    ...
    // Viewports list (the list is updated by calling ImGui::EndFrame or ImGui::PreRender)
