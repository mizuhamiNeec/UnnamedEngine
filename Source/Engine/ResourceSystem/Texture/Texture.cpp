...
bool Texture::LoadFromFile(D3D12* d3d12, SrvManager* shaderResourceViewManager,
	...
	ComPtr<ID3D12Resource> intermediateResource = UploadTextureData(
		d3d12->GetDevice(), d3d12->GetCommandList(), textureResource_,
		mipImages);
	...
	handle_    = shaderResourceViewManager->GetGPUDescriptorHandle(index);
	cpuHandle_ = shaderResourceViewManager->GetCPUDescriptorHandle(index);

	return true;
}

void Texture::FromOldTextureManager(const std::string& path) {
	filePath_ = path;
	auto data = TexManager::GetInstance()->GetTextureData(path);

	metadata_        = data->metadata;
	textureResource_ = data->resource;

	handle_    = data->srvHandleGPU;
	cpuHandle_ = data->srvHandleCPU;
}

D3D12_GPU_DESCRIPTOR_HANDLE Texture::GetShaderResourceView() const {
	return handle_;
}
...
bool Texture::CreateErrorTexture(
	D3D12*      d3d12,
...
ComPtr<ID3D12Resource> Texture::CreateTextureResource(ID3D12Device* device) {
	...
	resourceDesc.MipLevels = static_cast<UINT16>(metadata_.mipLevels);
	// MipMapの数
	resourceDesc.DepthOrArraySize = static_cast<UINT16>(metadata_.arraySize);
	// 奥行き or 配列Textureの配列数
	resourceDesc.Format = metadata_.format; // TextureのFormat
	resourceDesc.SampleDesc.Count = 1; // サンプリングカウント。1固定。
	resourceDesc.Dimension = static_cast<D3D12_RESOURCE_DIMENSION>(metadata_.
