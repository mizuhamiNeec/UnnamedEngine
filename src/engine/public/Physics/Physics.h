...
public:
	...
	DynamicBVH(DynamicBVH&& other) noexcept
		: nodes_(std::move(other.nodes_)), rootNode_(other.rootNode_),
...
namespace Physics {
	...
	bool RayIntersectsTriangle(const Vec3&     rayOrigin, const Vec3& rayDir,
	                           const Triangle& triangle, float&       outTime);
	float ComputeBoxPenetration(const Vec3& boxCenter, const Vec3& halfSize,
	                            const Vec3& hitPos, const Vec3&    hitNormal);
