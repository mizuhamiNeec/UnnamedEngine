...
bool PhysicsEngine::IntersectAABBWithTriangle(
	...
	const Vec3 closestPoint = ClosestPointOnTriangleToPoint(Vec3::zero, triangle);
	outHit.hitPos = aabbCenter + closestPoint;
	Debug::DrawRay(outHit.hitPos, normal, Vec4::magenta);
	const Vec3 penetration = ClosestPointOnAABBToPoint(closestPoint, aabb) - closestPoint;
